...
export class OrderController {
  ...
  async create(@Body() createOrderWithDetail: CreateOrderWithDetailDto) {
    ...
    for (const orderdetail of orderdetails) {
      ...
      let neworderDetail = new CreateOrderDetailDto(
        saveorder.id,
        idDish,
        price,
        quantity,
  ...
  findOrderByRes(@Param('status') status: number, @Request() userInfo) {
    if (userInfo.idUser) {
      ...
      throw new HttpException(
        'Chưa đăng nhập hoặc thiếu status trong query API',
  ...
  update(
    ...
    @Body('idOrder') idOrder: number,
    @Request() userInfo,
  ) {
    if (status != null && idOrder != null && userInfo.idUser != null) {
